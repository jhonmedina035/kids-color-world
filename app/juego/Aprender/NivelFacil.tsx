...
const ColorCard: React.FC<ColorCardProps> = React.memo(
  ({ data, onSpeak, isSpeaking }) => {
    // Determine el color de contraste para el texto
    const isLight = ["#FFEB3B", "#FF9800", "#E91E63", "#F44336"].includes(
      data.colorCode
    );
    const textColor =
      data.colorCode === "#000000" ? "#fff" : isLight ? "#000" : "#fff";
    const shadowColor =
      textColor === "#fff" ? "rgba(0, 0, 0, 0.9)" : "rgba(255, 255, 255, 0.7)";
...
const AprenderColores = () => {
  ...
  const renderItem = useCallback(
    ({ item }: { item: ColorData }) => (
      <ColorCard data={item} onSpeak={handleSpeak} isSpeaking={isSpeaking} />
    ),
    ...
    <View style={styles.mainContainer}>
      ...
      <View style={styles.navigationControls}>
        ...
        <TouchableOpacity
          style={[
            styles.navButton,
            currentIndex === COLORS.length - 1 && styles.navButtonDisabled,
          ]}
          onPress={handleNext}
